...
This is the log parser made with Java8, JPA 2.1 and Hibernate.
To run this project:
1. create an empty MySQL database called `accesslog`
2. create a user called `accesslog` with password `123456`

The database schema will be generated everytime you run the program (previous information will be wiped). 

You can download a .jar file of this project at: https://www.dropbox.com/s/eeabk63odaiclbo/parser.jar?dl=0

## Running

You may run the parser.jar as the problem below describes.
I also add an extra `--file` in order to allow you specify another location for the log file. If this param is suprimed, the parser will assume the `access.log` in the same folder as `parser.jar`.

## Queries:

Here's the queries specified by the problem below:

``` SQL
$ select ip, count(1) from log_entries where date between '2017-01-01.13:00:00' and '2017-01-01.14:00:00' group by ip having count(1) > 100;
```

``` SQL
$ select request from log_entries where ip = '192.168.228.188';
```
